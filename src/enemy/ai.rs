...
pub fn ai_movement(
    mut commands: Commands,
    mut enemy_qr: Query<(Entity, &mut EnemyVision, &Transform, &mut FacingDirection, &mut ActorState),
        (With<Enemy>, Without<MovementIntent>, Without<AiPath>)>,
    mut writer: MessageWriter<FindPath>,
) {
    for (enemy_entity, mut vision, tf, mut facing_direction, mut actor_state) in enemy_qr.iter_mut() {
        let enemy_pos = tf.translation.truncate();
        if let Some(player_pos) = vision.last_seen_pos {
            match vision.state {
                EnemyVisionState::Direct => {
                ...
                EnemyVisionState::PathFinding => {
                    ...
                    vision.last_seen_pos = None;
                    vision.state = EnemyVisionState::Idle;
                },
                EnemyVisionState::Idle => {
                    continue
                }
            }
        }
